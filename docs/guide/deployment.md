...
### OrbStack で Docker を動かす（macOS）

macOS（Apple Silicon / Intel）で Docker を動かすなら OrbStack が最も簡単です。

1. [OrbStack 公式サイト](https://orbstack.dev) からダウンロード（無料）
2. アプリを開くだけでセットアップ完了
3. メニューバーの OrbStack アイコンが緑色になれば準備完了
...
OrbStack は Docker Desktop のドロップイン代替品です。
`docker compose` コマンドはそのまま動作し、以降のすべての手順は Docker Desktop と同じです。
ファイル共有（バインドマウント）が高速でバッテリー消費も少ないため、長期運用に適しています。
